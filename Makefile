...
VERSION := $(shell python3 -c "from certs.attribute import __version__; print(__version__)")
...
pylint:
	pylint $(shell git ls-files certs/*.py)
flake8:
	flake8 certs --count --select=E9,F63,F7,F82 --show-source --statistics
	flake8 certs --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics
pytest:
	pytest --cov=certs --cov-report=term-missing --cov-report=xml --cov-report=html --cov-config=.coveragerc --cov-fail-under=100
pytest-clean:
	rm -rf .pytest_cache
